 ...
 *
 * Read-only summary the frontend uses to decide what to show:
 * current credit balance, plan, live pricing/costs, and free-plan usage.
 *
...
router.get("/access", async (req, res) => {
  try {
    ...
    // user genuinely shows up here with a verified email.
    ...
    return sendSuccess(res, {
      ...
      premiumUntil: profile.premiumUntil
        ? profile.premiumUntil.toDate().toISOString()
        : null,
      ...
      // USD pricing
      premiumMonthlyUSD: pricing.premiumMonthlyUSD,

      premiumYearlyUSD: pricing.premiumYearlyUSD,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      return sendSuccess(res, {
        reply: result.reply,
        creditsSpent: req.creditCost
      });

    } catch (err) {
      console.error(
        "POST /marketing/generate-batch error:",
        err.message
      );

      return sendError(
        res,
        "Failed to generate content. Your credits were not spent for this failed attempt.",
        502
      );
    }
  }
 ...
 *
 * A single caption/post for one platform.
 */
router.post(
  "/generate-caption",
  requireCredits("generateCaption"),
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "POST /marketing/generate-caption error:",
        err.message
      );
...
router.post(
  "/analyze",
  requireCredits("analyzeMarketing"),
 ...
 *
 * STUBBED — no image-generation AI provider is wired up yet.
 */
router.post(
  "/generate-image",
  async (req, res) => {
    return sendError(
      res,
      "AI image generation isn't connected yet — this is a placeholder endpoint for a future update. No credits were charged.",
      501
    );
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "POST /marketing/growth-coach error:",
        err.message
      );

      return sendError(
        res,
        "Failed to generate growth tips.",
        502
      );
    }
  }
